...
import { getStickerPackManifest } from '@talkapps/stickers-client';
...
/**
 * Queries Talk's Stickers API for each sticker pack enumerated in `inputFile`
...
export default async function compileStickerPackPartials(options: Required<FetchStickerDataOptions>) {
  ...
  /**
   * Limits concurrency of requests made to the Talk API to avoid throttling.
   */
  ...
  if (estimatedRequestCount > 0) {
    log.info(`Fetching ${log.chalk.yellow(estimatedRequestCount)} manifests from Talk...`);
  }
   ...
   *
   * 1. Determine if we have a cached manifest (the data we need from Talk) on
   *    disk for the pack.
   * 2. If not, fetch the manifest data from Talk and cache it to disk, then:
  ...
  await asyncQueue.addAll(R.map(([id, meta]) => async () => pRetry(async () => {
    ...
    if (await fs.pathExists(candidateSickerPackManifestPath)) {
      ...
      // Fetch the manifest for the current sticker pack from Talk using its
      // ID and key.
